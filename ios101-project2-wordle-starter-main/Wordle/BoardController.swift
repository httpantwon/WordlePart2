...
class BoardController: NSObject,
  ...
  func resetBoardWithCurrentSettings() {
    // START YOUR CODE HERE
      numTimesGuessed = 0 // numTimesGuessed should be set to 0
      collectionView.reloadData() // collectionView is reloaded
    // ...
    // END YOUR CODE HERE
  }
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      print(settings); //Tip
      if let numLetters = settings[kNumLettersKey] as? Int { //Tip 2. By hint 1, checks if a value exists
          numItemsPerRow = numLetters //Task 3
      }
    // ...
    // END YOUR CODE HERE
  }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
      //START YOUR CODE HERE
      print(settings);
      if let numOGuesses = settings[kNumGuessesKey] as? Int { // Tip 1
          numRows = numOGuesses
      }
    // ...
    // END YOUR CODE HERE
  ...
  // to check the before/after value of goalWord and see if it changes to the correct theme
    private func applyThemeSettings(with settings: [String: Any]) {
        // START YOUR CODE HERE
        if let rawTheme = settings[kWordThemeKey] as? String, //if not nil
           let theme = WordTheme(rawValue: rawTheme) {
            self.goalWord = WordGenerator.generateGoalWord(with: theme)
        }
        // ...
